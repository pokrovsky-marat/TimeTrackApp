...
const makeTokenAndSend = (user, res, statusCode) => {
  ...
  user.password = undefined; //Don't show password to client
...
exports.addUserToTemplate = cathAsyncErrors(async (req, res, next) => {
  //initilize user
  ...
  res.locals.user = freshUser; //Pass user to template
  next();
});
